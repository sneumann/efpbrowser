...
/* New in version 1.1.6efp, added by CSB U fT!!! */

static PyObject*
_replaceFill(ImagingObject* self, PyObject* args)
{
    int x, y;
    int trgSize, mapSize;
    PyObject * trgColor, * repColor;
    char trgInk[4]; // Target color
    char repInk[4]; // Replace color

    ImagingObject * mapObj;
    Imaging trg, map;
    
    if (!PyArg_ParseTuple(args, "O!OO", &Imaging_Type, &mapObj, &trgColor, &repColor))
      return NULL;
    
    map = mapObj->image;
    trg = self->image;

    // Convert color tuple to 32-bit character array
    if (!getink(trgColor, map, trgInk))
        return NULL;
    if (!getink(repColor, trg, repInk))
        return NULL;

    trgSize = trg->xsize * trg->ysize;
    mapSize = map->xsize * trg->ysize;

    // Ensure total number of pixels for both images are the same
    if (trgSize != mapSize)
        return NULL;

    if (trg->image8) {
        for (y = 0; y < trg->ysize; y++) {
            for (x = 0; x < trg->xsize; x++) {
                if (map->image8[y][x] == trgInk[0]) {
                    trg->image8[y][x] = repInk[0];
                }
            }
        }
    }
        ...
        for (y = 0; y < trg->ysize; y++) {
            for (x = 0; x < trg->xsize; x++) {
                if (map->image32[y][x] == *((INT32*)trgInk)) {
                    trg->image32[y][x] = *((INT32*)repInk);
                }
            }
        }
    }
...
#endif
    {"replaceFill", (PyCFunction)_replaceFill, 1},  /* ADDED by CSB UofT !!! */
